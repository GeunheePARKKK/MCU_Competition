...
public:
    PsdIndicatorFrame update(const UiSnapshot &s, uint32_t now) {
        ...
        if (!alarm) {
            ...
            } else if (lastTrain_ != s.trainState &&
                       (s.trainState == TRAIN_STATE_DOOR_OPENING ||
                        // train_motor_sketch skips the TRAIN door step: COASTING -> WAIT_PSD_OPEN.
                        (s.trainState == TRAIN_STATE_WAIT_PSD_OPEN &&
                         lastTrain_ != TRAIN_STATE_DOOR_OPENING))) {
