...
/* train_motor_sketch: 모터 출력 상태를 LCD 문구에 그대로 반영한다. */
#if TRAIN_MOTOR_ENABLED
#define UI_MOTOR_NORMAL "MOTOR OUTPUT ON"
#define UI_MOTOR_SHORT "MOT:ON"
#else
#define UI_MOTOR_NORMAL "SIM: MOTORS OFF"
#define UI_MOTOR_SHORT "MOT:OFF"
#endif
...
inline void uiFormatLcd(const UiSnapshot &s, uint32_t now, char *top, char *bottom) {
    ...
    if (count != COUNTDOWN_INACTIVE) {
        if (s.normal) {
            ...
            snprintf_P(bottom, 17, PSTR(UI_MOTOR_NORMAL));
        } else {
    ...
    if (s.trainLocal) {
        ...
        if (!page)
            ...
            snprintf_P(bottom, 17, PSTR("V%u D%c P%u " UI_MOTOR_SHORT), unsigned(s.closed), uiDoor(s.doorCommand), unsigned(s.peerClosed));
    } else {
