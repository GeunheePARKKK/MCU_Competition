...
/* 설정이 없으면 원래 순서(열차 문 먼저)로 동작한다. */
#ifndef TRAIN_DOOR_STEP_ENABLE
#define TRAIN_DOOR_STEP_ENABLE 1
#endif
...
#if !TRAIN_DOOR_STEP_ENABLE
    /* 열차 문 단계를 건너뛰므로 열차 문 명령은 열림으로 바꾸지 않는다. */
    if (outputs->door_command == TRAIN_DOOR_OPEN)
    {
        outputs->door_command = TRAIN_DOOR_CLOSE;
    }
#endif
...
#else
                    /* 정차 재확인 통과 -> 열차 문을 기다리지 않고 바로 PSD 열기 */
...
#else
                /* 카운트다운 끝 -> 열차 문을 기다리지 않고 바로 PSD 닫기 */
