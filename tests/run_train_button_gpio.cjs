...
const train = true;
// TRAIN one-button protocol. Original PSD runner remains separate.
...
for (let index = 0; index < 10; ++index) {
  ...
  assert.equal(lines.filter(x => x.startsWith('SELECT ')).at(-1), 'SELECT ' + ((index + 1) % 10));
}
console.log('P16/P15 all pulse widths, motor PWM, 10 entries, hold lease and selection wrap: PASS');
...
console.log('TRAIN ONE-BUTTON GPIO TESTS PASS; physical P4 control, no hardware upload');
